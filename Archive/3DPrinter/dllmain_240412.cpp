...
ProError executeBondpadPbAction(char* dialog, char* component, ProAppData appdata)
{
	...
	status = ProUILayoutHide(paramsDialog, layout_trace_margin);
...
int placeCurveAction(char* dialog, char* component, ProAppData appdata)
{
	...
	double xOffsetVal, yOffsetVal, fanoutBoundaryVal;
	...
	status = ProUIInputpanelDoubleGet(paramsDialog, ip_trace_margin, &fanoutBoundaryVal);

	vector<fanout_pnt> fanoutMidpointsCoords{};
	for (int i = 0; i < nodePairs.size(); ++i)
	{
		if (nodePairs[i].size() == 2)	// temparory purpose
		{
			...
			for (auto &point : fanoutMidpointsCoords)
			{
				...
				{
					ProFeature pnt1Feat, pnt2Feat;
					bool isFeatFound = false;
					isFeatFound = GetFeatureByName(CurMdl1, _point1Name, pnt1Feat);
					if (!isFeatFound)
						return -1;
					isFeatFound = GetFeatureByName(CurMdl1, _point2Name, pnt2Feat);
					if (!isFeatFound)
						return -1;

					vector<ProGeomitem> pnt1GeomItems, pnt2GeomItems;
					GetGeomItems(CurMdl1, pnt1Feat, pnt1GeomItems);
					GetGeomItems(CurMdl1, pnt2Feat, pnt2GeomItems);
					
					ProPoint pnt1, pnt2;
					ProGeomitemToPoint(&pnt1GeomItems[0], &pnt1);
					ProGeomitemToPoint(&pnt2GeomItems[0], &pnt2);

					Pro3dPnt pnt1CoOrd, pnt2CoOrd;
					status = ProPointCoordGet(pnt1, pnt1CoOrd);
					status = ProPointCoordGet(pnt2, pnt2CoOrd);
			...
			fanoutMidpointsCoords.push_back({ pnt3D[0], pnt3D[1], pnt3D[2],_point1Name, _point2Name });
			status = ProModelitemHide(&feats[1]);
		}
	}

	for (int i = 0; i < fanoutMidpointsCoords.size(); ++i)
	{
		ProPoint3d pnt3D;
		pnt3D[0] = fanoutMidpointsCoords[i].x;
		pnt3D[1] = fanoutMidpointsCoords[i].y;
		pnt3D[2] = fanoutMidpointsCoords[i].z;
		...
		surfaceUV[0] = surfaceUV[0] + (xOffsetVal * 0.001);
		surfaceUV[1] = surfaceUV[1] + yOffsetVal;
		...
		ProName wFoMpntName;
		sprintf(cFoMpntName, "FO_MP_%s_%s", fanoutMidpointsCoords[i].pnt1Name.c_str(), fanoutMidpointsCoords[i].pnt2Name.c_str());
		...
		for (int j = 0; j < fanoutMidpoints.size(); ++j)
		{
			...
			if (string(cName).find(cFoMpntName) == string::npos)
			{
				...
				if (distBwPnts < fanoutBoundaryVal)
		...
		ProFeature csoFeat, csoFeat1;
		ProFeature* feats1, *feats2;
		std::vector<ProSelection> UdfInputSel;
		PointData curPointData = {};
		string point1 = fanoutMidpointsCoords[i].pnt1Name;
		//string point2 = sMiddleCurveName;
		string point2 = string(cBendPointName);
		int surfaceid = 41245;
		ProFeature curveFeat, offsetCurve1Feat, offsetCurve2Feat;
		createCurveBw2Points(CurMdl1, point1, csoFeat, curPointData, point2, csoFeat1, status, surfaceid, comp_path, vectPointData, UdfInputSel, curveFeat);
		status = ProGroupFeaturesCollect(&curveFeat, &feats1);//Give only non suppressed entities
		status = ProModelitemHide(&feats1[1]);
		...
		//point1 = sMiddleCurveName;
		point1 = string(cBendPointName);
		point2 = fanoutMidpointsCoords[i].pnt2Name;
		UdfInputSel.clear();
		vectPointData.clear();
		createCurveBw2Points(CurMdl1, point1, csoFeat, curPointData, point2, csoFeat1, status, surfaceid, comp_path, vectPointData, UdfInputSel, curveFeat);
		status = ProGroupFeaturesCollect(&curveFeat, &feats2);//Give only non suppressed entities
		status = ProModelitemHide(&feats2[1]);
