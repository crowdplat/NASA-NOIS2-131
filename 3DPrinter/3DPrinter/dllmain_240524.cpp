...
ProError executeFanoutPbAction(char* dialog, char* component, ProAppData appdata)
{
	...
	status = ProSelect((char*)"surface",-1,NULL,NULL,selEnv,NULL,&surfSels,&nSurfSels);
...
void updateBondpadAction(char* dialog, char* component, ProAppData appdata)
{
	...
	double height, width, ofstX, ofstY, traceMargin, angle;
	...
	status = ProDimensionValueSet(&vecFeatDims[1], -angle);
	vecFeatDims.clear();
