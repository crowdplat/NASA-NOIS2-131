...
ProError executeBondpadPbAction(char* dialog, char* component, ProAppData appdata)
{
	...
	strcat(bondpadUdfpath, "\\text\\UDF_LATEST\\bondpa_tilted_with_dims.gph.1");
...
ProError updateBondpadAction(char* dialog, char* component, ProAppData appdata)
{
	...
	ProBoolean is_regened_negative = PRO_B_FALSE;
	status = ProDimensionIsRegenednegative(&vecFeatDims[1], &is_regened_negative);
	...
	status = ProDimensionValueGet(&vecFeatDims[0], &existAngle);
